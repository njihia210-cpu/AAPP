...
## 5. Host-Konto anlegen — direkt im Browser
...
Der Endpunkt `/api/cron/reminders` verschickt, was gerade fällig ist. Er
...
`/api/cron/reminders`, das `CRON_SECRET` reist als Kopfzeile mit statt in der
...
```
https://<deine-domain>/api/cron/reminders?secret=<CRON_SECRET>&window=15
```
