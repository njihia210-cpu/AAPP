...
import { seedEinmalig } from './_lib/seed.js';
...
export default api(async function handler(req, res) {
  ...
  if (wunsch === 'login') {
    ...
    setSessionCookie(res, await createSession(username, bleiben), bleiben);
  ...
  if (wunsch === 'password') {
    ...
    const bleiben = req.body?.bleiben !== false;
    await destroyAllSessions(record.username);
    setSessionCookie(res, await createSession(record.username, bleiben), bleiben);
    return res.status(200).json({ ok: true });
  }
