...
FL = K['flexibel']
add(k, 'Was ist eine flexible Pensionierung?',
    'Der Rücktritt lässt sich vorziehen, aufschieben oder in Teilen vollziehen – die Rente beginnt nicht '
    'zwingend mit dem Referenzalter und nicht zwingend ganz', 'e')
add(k, 'Zwischen welchen Altersjahren kann die AHV-Rente bezogen werden?',
    f"{FL['bezug_zwischen']}", 'm')
add(k, 'Was ist ein Teilvorbezug der AHV-Rente?',
    f"Es wird nur ein Teil der Rente vorbezogen – {FL['teilbezug_von']} bis {FL['teilbezug_bis']} Prozent; "
    'der Rest folgt später', 'm')
add(k, 'Wie oft lässt sich der vorbezogene Anteil erhöhen?',
    'Einmal – danach ist der Anteil festgelegt', 'p')
add(k, 'Lassen sich Vorbezug und Aufschub kombinieren?',
    'Ja – ein Teil der Rente kann vorbezogen und ein anderer aufgeschoben werden', 's')
add(k, 'Was verlangt das Gesetz von der Pensionskasse bei der Teilpensionierung?',
    f"Sie muss den gleitenden Übertritt in mindestens {FL['bvg_schritte']} Schritten ermöglichen", 'm')
add(k, 'Warum ist die Teilpensionierung steuerlich interessant?',
    'Weil sich Kapitalbezüge über mehrere Jahre staffeln lassen und die Progression jedes Mal neu beginnt', 'p')
add(k, 'Was spricht sonst noch für eine Teilpensionierung?',
    'Das Einkommen sinkt schrittweise statt auf einen Schlag, und die Sparjahre laufen für den Restlohn weiter', 's')
...
# ================================================== Eingliederung
k = kap('vbv-iv-eingliederung', 'Eingliederungsmassnahmen der IV',
        'Was die IV unternimmt, bevor sie eine Rente spricht')
EG = I['eingliederung']
add(k, 'Warum steht die Eingliederung vor der Rente?',
    'Weil eine erhaltene Erwerbsfähigkeit mehr wert ist als jede Rente – für die versicherte Person wie für die '
    'Versicherung', 'e')
add(k, 'Welche fünf Arten von Eingliederungsmassnahmen kennt die IV?', EG['arten'], 'e')
add(k, 'Was ist die Früherfassung?', EG['frueherfassung'], 'm')
add(k, 'Wer darf eine Früherfassung melden?', EG['frueherfassung_melder'], 'm')
add(k, 'Was ist die Frühintervention?',
    f"Rasches Handeln nach der Anmeldung, um den Arbeitsplatz zu erhalten – längstens "
    f"{EG['frueintervention_dauer_monate']} Monate", 'm')
add(k, 'Welche Frühinterventionsmassnahmen gibt es?', EG['frueintervention_massnahmen'], 'm')
add(k, 'Wozu dienen die Integrationsmassnahmen?',
    'Sie bereiten auf die berufliche Eingliederung vor, wenn die Arbeitsfähigkeit dafür noch nicht reicht', 'm')
add(k, 'Welche Integrationsmassnahmen gibt es?', EG['integrationsmassnahmen'], 's')
add(k, 'Nenne die beruflichen Massnahmen der IV.', EG['berufliche_massnahmen'], 'm')
add(k, 'Was leistet die Berufsberatung?',
    'Beratungsgespräche, Abklärungen und Tests, um eine gesundheitlich passende Tätigkeit zu finden', 'e')
add(k, 'Was zahlt die IV bei der erstmaligen beruflichen Ausbildung?',
    'Nur die invaliditätsbedingten Mehrkosten – nicht die ganze Ausbildung', 'p')
add(k, 'Was zahlt die IV bei einer Umschulung?',
    'Sämtliche Kosten – die Umschulung erhält oder verbessert die Erwerbsfähigkeit', 'p')
add(k, 'Was leistet die Arbeitsvermittlung?',
    'Unterstützung beim Erhalt oder bei der Suche einer Stelle im ersten Arbeitsmarkt – mit Anpassungen, '
    'Job-Coaching und Hilfe beim Bewerben', 'm')
add(k, 'Was ist die Kapitalhilfe?',
    'Mittel für eine selbstständige Tätigkeit, in der Regel als verzinsliches und rückzahlbares Darlehen', 's')
add(k, 'Was ist der Einarbeitungszuschuss?',
    f"Ein befristeter Beitrag an den Arbeitgeber, solange die Leistung noch vermindert ist – längstens "
    f"{EG['einarbeitungszuschuss_tage']} Tage", 's')
add(k, 'Was ist die Entschädigung für Beitragserhöhungen?',
    f"Sie fängt den Arbeitgeber auf, wenn die vermittelte Person innert {EG['beitragserhoehung_jahre']} Jahren "
    'erneut arbeitsunfähig wird', 'p')
add(k, 'Für wen zahlt die IV medizinische Massnahmen?',
    'Für die Behandlung anerkannter Geburtsgebrechen bis zum 20. Altersjahr', 'm')
add(k, 'Was sind Hilfsmittel der IV?',
    'Gegenstände, die Beruf und Alltag möglich machen – Prothesen, Hörgeräte, Rollstühle, Anpassungen am '
    'Arbeitsplatz', 'e')
add(k, 'Was sichert das IV-Taggeld?',
    f"Den Lebensunterhalt während der Eingliederung, ab dem {EG['taggeld_ab_alter']}. Altersjahr", 'm')
add(k, 'Wann erlischt der Anspruch auf Eingliederungsmassnahmen?',
    'Beim Erreichen des Referenzalters', 's')
add(k, 'Welche Pflicht trifft die versicherte Person?',
    'Die Schadenminderungspflicht – sie muss an der Eingliederung mitwirken und die Kosten in Grenzen halten', 's')
add(k, 'Warum ist die frühe Meldung so wichtig?',
    'Weil sich ein Arbeitsplatz nur erhalten lässt, solange er besteht – und die Rente ohnehin frühestens sechs '
    'Monate nach der Anmeldung fliesst', 'p')
add(k, 'Was heisst das für die Beratung?',
    'Bei drohender Erwerbsunfähigkeit zuerst die Eingliederung ansprechen, erst danach über Renten und Lücken '
    'reden', 'p')
...
U = K['uvg']
add(k, 'Definition eines Unfalls?',
    'Die plötzliche, nicht beabsichtigte schädigende Einwirkung eines ungewöhnlichen äusseren Faktors auf den '
    'menschlichen Körper, die eine Beeinträchtigung der Gesundheit oder den Tod zur Folge hat', 'e')
add(k, 'Welche Merkmale müssen für einen Unfall zusammen erfüllt sein?',
    'Plötzlich, nicht beabsichtigt, schädigend, ungewöhnlicher äusserer Faktor, Einwirkung auf den menschlichen '
    'Körper – fehlt eines, ist es kein Unfall', 'm')
add(k, 'In welchem Gesetz steht die Definition des Unfalls?',
    'Im ATSG, dem Bundesgesetz über den Allgemeinen Teil des Sozialversicherungsrechts', 'm')
add(k, 'Wie ist die Krankheit demgegenüber definiert?',
    'Jede Beeinträchtigung der Gesundheit, die nicht Folge eines Unfalles ist und eine Behandlung erfordert '
    'oder arbeitsunfähig macht', 'm')
add(k, 'Was sind unfallähnliche Körperschädigungen?',
    'Eine abschliessende Liste von Verletzungen, die auch ohne ungewöhnlichen äusseren Faktor als Unfall gelten', 's')
add(k, 'Welche Verletzungen stehen auf dieser Liste?',
    'Knochenbrüche, Verrenkungen von Gelenken, Meniskusrisse, Muskelrisse, Muskelzerrungen, Sehnenrisse, '
    'Bandläsionen und Trommelfellverletzungen', 'p')
add(k, 'Warum ist die Abgrenzung Unfall oder Krankheit so wichtig?',
    'Weil sie entscheidet, welche Versicherung zahlt – und die Leistungen des UVG deutlich besser sind als jene '
    'der Krankenversicherung', 's')
...
add(k, 'Sind Leistungen aus einer reinen Risikoversicherung der Säule 3b steuerbar?',
    'Ja – die Police ist nicht rückkaufsfähig, deshalb wird die Todesfallsumme mit der Kapitalleistungssteuer '
    'zum Vorsorgetarif erfasst', 'p')

# ============================================ Besteuerung nach Säule
k = kap('vbv-steuermatrix', 'Besteuerung nach Säule und Leistung',
        'Welche Steuer trifft welche Leistung')
MX = ST['matrix']
add(k, 'Welche drei Steuerarten sind hier zu unterscheiden?',
    'Die Einkommenssteuer, die Kapitalleistungssteuer zum Sondersatz und die kantonale Erbschaftssteuer', 'e')
add(k, 'Wie wird eine Rente aus der 2. Säule besteuert?', MX['saeule2_rente'], 'e')
add(k, 'Wie wird eine Kapitalleistung aus der 2. Säule besteuert?', MX['saeule2_kapital'], 'e')
add(k, 'Wie wird die Ablaufleistung einer gemischten Lebensversicherung der Säule 3a besteuert?',
    MX['saeule3a_gemischt'], 'm')
add(k, 'Wie wird der Tod während der Laufzeit bei einer gemischten 3a-Police besteuert?',
    'Ebenfalls mit der Kapitalleistungssteuer – die Leistung fällt nicht in den Nachlass', 'm')
add(k, 'Wie wird eine reine Erwerbsunfähigkeitsrente aus der Säule 3a besteuert?',
    MX['saeule3a_eu_rente'], 'm')
add(k, 'Wie wird eine reine Todesfallversicherung der Säule 3a besteuert?',
    MX['saeule3a_todesfallversicherung'], 'm')
add(k, 'Wie wird eine reine Erwerbsunfähigkeitsrente aus der Säule 3b besteuert?',
    MX['saeule3b_eu_rente'], 'm')
add(k, 'Wie wird eine reine Todesfallkapitalversicherung der Säule 3b besteuert?',
    'Mit der Kapitalleistungssteuer zum Vorsorgetarif – die Police ist nicht rückkaufsfähig', 'm')
add(k, 'Wie wird die Ablaufleistung einer gemischten Lebensversicherung der Säule 3b besteuert?',
    'Steuerfrei, sofern die Bedingungen erfüllt sind', 'm')
add(k, 'Welche Bedingungen machen die 3b-Kapitalversicherung steuerfrei?',
    ST['kapitalversicherung_steuerfrei'], 'p')
add(k, 'Wie wird der Tod während der Laufzeit bei einer gemischten 3b-Police behandelt?',
    MX['saeule3b_gemischt_tod'], 'p')
add(k, 'Wo liegt der Unterschied zwischen 3a und 3b im Todesfall?',
    'In der Säule 3a fällt die Kapitalleistungssteuer an; in der Säule 3b bleibt die Leistung '
    'einkommenssteuerfrei, dafür kann die Erbschaftssteuer greifen', 's')
add(k, 'Was haben alle Renten gemeinsam?',
    'Sie werden als Einkommen besteuert – gleich aus welcher Säule sie stammen', 's')
add(k, 'Was haben alle Kapitalleistungen aus gebundener Vorsorge gemeinsam?',
    'Sie werden gesondert vom übrigen Einkommen zum Sondersatz besteuert', 's')
add(k, 'Warum ist die reine Risikoversicherung der Säule 3b nicht steuerfrei?',
    'Weil die Steuerfreiheit nur rückkaufsfähigen Kapitalversicherungen zusteht – eine reine Risikoversicherung '
    'hat keinen Rückkaufswert', 'p')
add(k, 'Woran erkennt man, ob Kapitalleistungssteuer oder Steuerfreiheit gilt?',
    'An der Bindung: gebundene Vorsorge wird beim Bezug besteuert, die freie 3b-Kapitalversicherung bleibt '
    'unter Bedingungen frei', 'p')
add(k, 'Merksatz zur ganzen Übersicht?',
    'Renten als Einkommen, Kapital gesondert – nur die rückkaufsfähige 3b-Kapitalversicherung fällt heraus', 's')
...
TK = K['bvg']['todesfallkapital_ordnung']
add(k, 'Was ist ein Todesfallkapital in der Pensionskasse?',
    'Eine einmalige Leistung beim Tod vor der Pensionierung – meist das nicht verbrauchte Altersguthaben, wenn '
    'keine oder nur kleine Renten laufen', 'e')
add(k, 'Wer steht beim Todesfallkapital der Pensionskasse zuoberst?',
    'Der Ehegatte oder eingetragene Partner und die Kinder mit Anspruch auf eine Waisenrente', 'm')
add(k, 'Wer folgt beim Todesfallkapital an zweiter Stelle?',
    'Personen, die in erheblichem Masse unterstützt wurden, oder die Person aus einer fünfjährigen '
    'Lebensgemeinschaft beziehungsweise mit gemeinsamen Kindern', 'm')
add(k, 'Wer folgt beim Todesfallkapital an dritter und vierter Stelle?',
    'Die übrigen Kinder, bei deren Fehlen die Eltern, bei deren Fehlen die Geschwister – zuletzt die übrigen '
    'gesetzlichen Erben ohne das Gemeinwesen', 'm')
add(k, 'Nenne die Reihenfolge beim Todesfallkapital der Pensionskasse vollständig.',
    '1. Ehegatte und rentenberechtigte Kinder, 2. unterstützte Personen und Lebensgemeinschaft, '
    '3. übrige Kinder, sonst Eltern, sonst Geschwister, 4. übrige gesetzliche Erben', 'p')
add(k, 'Welcher Artikel regelt das Todesfallkapital im BVG?', TK['grundlage'], 'p')
add(k, 'Was muss der Lebenspartner in der Pensionskasse zwingend tun?',
    'Er muss der Vorsorgeeinrichtung schriftlich gemeldet werden – ohne Meldung besteht kein Anspruch', 's')
add(k, 'Wer bestimmt die genaue Ordnung in der Pensionskasse?',
    'Das Reglement der Vorsorgeeinrichtung, im Rahmen des Gesetzes – deshalb lohnt sich der Blick ins Reglement', 's')
add(k, 'Wie ist die Begünstigung im UVG geordnet?',
    'Gar nicht wählbar: das Gesetz bestimmt die Hinterlassenen abschliessend – überlebender Ehegatte, Kinder und '
    'unter Bedingungen der geschiedene Ehegatte', 'm')
add(k, 'Gibt es im UVG ein Todesfallkapital?',
    'Nein – das UVG zahlt Hinterlassenenrenten, keine Kapitalleistung zur freien Begünstigung', 's')
add(k, 'Vergleiche die Begünstigung in den drei Säulen.',
    '2. Säule: gesetzliche Gruppen, das Reglement bestimmt Näheres · Säule 3a: die Reihenfolge der BVV 3 · '
    'Säule 3b: freie Wahl, Pflichtteile vorbehalten · UVG: gesetzlich fixiert, keine Wahl', 'p')
...
k = kap('vbv-vorsorgeauftrag', 'Vorsorgeauftrag und Patientenverfügung',
        'Selbstbestimmung bei Urteilsunfähigkeit')
VA = K['vorsorgeauftrag']
add(k, 'Was ist ein Vorsorgeauftrag?',
    'Die Bestimmung, wer bei eigener Urteilsunfähigkeit die persönlichen und finanziellen Angelegenheiten '
    'übernimmt', 'e')
add(k, 'In welchem Gesetz steht der Vorsorgeauftrag?',
    'Im Erwachsenenschutzrecht des ZGB', 'm')
add(k, 'Welche Form braucht ein Vorsorgeauftrag?', VA['form'], 'm')
add(k, 'Welche drei Bereiche kann ein Vorsorgeauftrag umfassen?', VA['bereiche'], 'm')
add(k, 'Wann wird ein Vorsorgeauftrag wirksam?', VA['wirksam'], 's')
add(k, 'Was prüft die KESB bei der Validierung?',
    'Ob die Form stimmt, ob wirklich Urteilsunfähigkeit vorliegt und ob die beauftragte Person geeignet und '
    'bereit ist', 'p')
add(k, 'Lässt sich ein Vorsorgeauftrag widerrufen?', VA['widerruf'], 's')
add(k, 'Wo lässt sich ein Vorsorgeauftrag registrieren?',
    'Der Hinterlegungsort kann beim Zivilstandsamt eingetragen werden – gefunden wird nur, was auffindbar ist', 'p')
add(k, 'Was ist eine Patientenverfügung?',
    'Die Festlegung, welchen medizinischen Massnahmen man bei Urteilsunfähigkeit zustimmt – und wer im '
    'Behandlungsfall für einen entscheidet', 'e')
add(k, 'Welche Form braucht eine Patientenverfügung?',
    'Schriftlich, datiert und unterschrieben – von Hand geschrieben muss sie nicht sein', 'm')
add(k, 'Was unterscheidet die beiden Dokumente?',
    'Die Patientenverfügung regelt die medizinische Behandlung, der Vorsorgeauftrag alles Übrige – Person, '
    'Vermögen und Vertretung', 's')
add(k, 'Was gilt, wenn kein Vorsorgeauftrag besteht?',
    'Das gesetzliche Vertretungsrecht des Ehegatten deckt nur die alltäglichen Angelegenheiten; darüber hinaus '
    'errichtet die KESB eine Beistandschaft', 's')
add(k, 'Warum genügt die Ehe allein nicht?',
    'Weil der Ehegatte ohne Auftrag nur das Alltägliche erledigen darf – für den Verkauf einer Liegenschaft '
    'braucht es die Behörde', 'p')
add(k, 'Warum gehören beide Dokumente in eine Vorsorgeberatung?',
    'Weil sie nichts kosten, jede Lücke im Ernstfall teuer wird und der Anlass – Heirat, Kind, Immobilie – '
    'ohnehin auf dem Tisch liegt', 's')

# ================================================== Erbrecht
k = kap('vbv-erbrecht', 'Erbrecht in der Beratung', 'Wer erbt was – und in welcher Ordnung')
add(k, 'Wie ist die gesetzliche Erbfolge aufgebaut?', ER['parentelen'], 'm')
add(k, 'Was bedeutet das Parentelsystem in einem Satz?',
    'Solange in einer Parentel jemand lebt, erben die folgenden Parentelen nichts', 's')
add(k, 'Wie viel erbt der Ehegatte neben Nachkommen?', 'Die Hälfte des Nachlasses', 'm')
add(k, 'Wie viel erbt der Ehegatte neben dem elterlichen Stamm?', 'Drei Viertel des Nachlasses', 'm')
add(k, 'Wie viel erbt der Ehegatte, wenn niemand sonst da ist?', 'Den ganzen Nachlass', 'm')
add(k, 'Welcher Güterstand gilt ohne Ehevertrag?',
    'Die Errungenschaftsbeteiligung – Gütergemeinschaft und Gütertrennung brauchen einen öffentlich '
    'beurkundeten Ehevertrag', 'm')
add(k, 'Was kommt beim Tod zuerst: Güterrecht oder Erbrecht?',
    'Zuerst die güterrechtliche Auseinandersetzung – in den Nachlass fällt nur, was danach dem Verstorbenen '
    'gehört', 'p')
add(k, 'Welche Testamentsformen gibt es?', ER['testament_formen'], 'm')
add(k, 'Was braucht ein eigenhändiges Testament zwingend?',
    'Es muss ganz von Hand geschrieben, datiert und unterschrieben sein – am Computer geschrieben ist es '
    'ungültig', 'p')
add(k, 'Was ist ein Erbvertrag?',
    'Eine öffentlich beurkundete Vereinbarung mit zwei Zeugen – anders als das Testament bindet sie beide '
    'Seiten', 's')
add(k, 'Wann wird ein Erbvertrag dem Testament vorgezogen?',
    'Wenn eine Zusage verbindlich sein soll – etwa ein Erbverzicht oder die Absicherung eines Partners', 'p')
add(k, 'Was erbt der Konkubinatspartner von Gesetzes wegen?',
    'Nichts – ohne Testament oder Erbvertrag geht er leer aus, und er zahlt meist die höchste Erbschaftssteuer', 's')
add(k, 'Wie lässt sich der Ehegatte maximal begünstigen?',
    'Mit dem Ehevertrag die ganze Errungenschaft zuweisen und im Testament die freie Quote – die Pflichtteile '
    'der Kinder bleiben', 'p')
add(k, 'Warum gehört das Erbrecht in die Vorsorgeberatung?',
    'Weil Vorsorgeleistungen und Nachlass zusammen wirken: Was an der Erbteilung vorbeigeht, verschiebt das '
    'Ergebnis für alle Beteiligten', 's')

# ================================================== Heirat
k = kap('vbv-heirat', 'Was die Heirat verändert', 'Wirkung in allen Säulen und im Erbrecht')
add(k, 'Was ändert die Heirat in der 1. Säule bei den Renten?',
    f"Die beiden Altersrenten werden zusammen auf {K['ahv']['plafond_prozent']} der Maximalrente plafoniert", 'm')
add(k, 'Was bewirkt das Splitting bei Verheirateten?',
    'Die während der Ehe erzielten Einkommen werden hälftig geteilt und beiden je zur Hälfte angerechnet', 'm')
add(k, 'Wann wird gesplittet?',
    'Wenn beide pensioniert sind, bei einer Scheidung oder beim Tod eines Ehegatten', 's')
add(k, 'Welcher Anspruch entsteht in der 1. Säule durch die Heirat?',
    'Der Anspruch auf eine Witwen- oder Witwerrente – das Konkubinat kennt ihn nicht', 'm')
add(k, 'Was gilt für die Beiträge des nichterwerbstätigen Ehegatten?',
    'Sie gelten als bezahlt, wenn der erwerbstätige Ehegatte mindestens den doppelten Mindestbeitrag entrichtet', 's')
add(k, 'Was ändert die Heirat in der 2. Säule?',
    'Es entsteht der Anspruch auf eine Ehegattenrente, und der Ehegatte rückt beim Todesfallkapital an die '
    'erste Stelle', 'm')
add(k, 'Wofür braucht es in der 2. Säule neu die Unterschrift des Ehegatten?',
    'Für den Kapitalbezug statt der Rente und für einen Vorbezug oder eine Verpfändung für Wohneigentum', 'm')
add(k, 'Was passiert bei einer Scheidung mit der 2. Säule?',
    'Die während der Ehe geäufneten Guthaben werden geteilt – der Vorsorgeausgleich', 's')
add(k, 'Was ändert die Heirat in der Säule 3a?',
    'Der Ehegatte rückt zwingend an die erste Stelle der Begünstigtenordnung – vor die Nachkommen', 'm')
add(k, 'Was ändert die Heirat in der Säule 3b?',
    'An der freien Begünstigung nichts – aber der Ehegatte hat neu einen Pflichtteil, der zu beachten ist', 's')
add(k, 'Was ändert die Heirat im Erbrecht?',
    'Der Ehegatte wird gesetzlicher Erbe und ist pflichtteilsgeschützt – neben Nachkommen erbt er die Hälfte', 'm')
add(k, 'Welcher Güterstand entsteht mit der Heirat?',
    'Die Errungenschaftsbeteiligung, sofern kein Ehevertrag etwas anderes bestimmt', 'm')
add(k, 'Was bleibt bei der Heirat unverändert gültig?',
    'Ein früher errichtetes Testament – es wird durch die Heirat nicht aufgehoben, kann aber neu Pflichtteile '
    'verletzen', 'p')
add(k, 'Welche vier Punkte gehören nach einer Heirat auf die Traktandenliste?',
    'Begünstigung in der 3. Säule und der Pensionskasse melden, Testament prüfen, Güterstand klären, '
    'Vorsorgeauftrag und Patientenverfügung errichten', 'p')
add(k, 'Was ist der grösste Unterschied zwischen Ehe und Konkubinat in der Vorsorge?',
    'Die Ehe schafft Ansprüche von Gesetzes wegen; im Konkubinat entsteht ohne Meldung, Testament und Vertrag '
    'kein einziger', 's')
add(k, 'Was ändert eine eingetragene Partnerschaft?',
    'Sie ist der Ehe in der Vorsorge und im Erbrecht gleichgestellt', 'm')
...
# ============================================ Geldwäscherei
k = kap('vbv-gwg', 'Geldwäscherei und GwG', 'Sorgfaltspflichten beim Abschluss')
GW = K['gwg']
add(k, 'Wofür steht die Abkürzung GwG?', 'Geldwäschereigesetz', 'e')
add(k, 'Was will das Geldwäschereigesetz verhindern?',
    'Dass Vermögen aus Verbrechen in den Finanzkreislauf gelangt – und es bekämpft die Terrorismusfinanzierung', 'e')
add(k, 'Wer gilt als Finanzintermediär?',
    'Wer berufsmässig fremde Vermögenswerte annimmt, aufbewahrt oder bei ihrer Anlage oder Übertragung hilft – '
    'Banken, Lebensversicherer, Vermögensverwalter, Treuhänder, Spielbanken', 'e')
add(k, 'Wer beaufsichtigt die Finanzintermediäre?', GW['aufsicht'], 'm')
add(k, 'Wofür steht die Abkürzung SRO?', 'Selbstregulierungsorganisation', 'e')
add(k, 'Welche Selbstregulierungsorganisation gilt für die Lebensversicherer?', GW['sro_leben'], 'm')
add(k, 'Wer beaufsichtigt die Spielbanken nach dem GwG?', GW['spielbanken'], 'p')
add(k, 'Wer muss beim Abschluss einer Lebensversicherung identifiziert werden?', GW['identifikation'], 'e')
add(k, 'Womit wird die Vertragspartei identifiziert?',
    'Mit einem amtlichen Ausweis mit Foto – Kopie zu den Akten', 'm')
add(k, 'Ab welcher Höhe muss identifiziert werden?',
    f"Wenn die Prämien pro Vertrag innerhalb von {GW['schwelle_jahre']} Jahren {CHF(GW['schwelle_leben'])} Franken übersteigen", 'm')
add(k, 'Welche Versicherungen fallen unter das GwG?', GW['erfasst'], 'm')
add(k, 'Welche Versicherungen fallen nicht darunter?',
    'Reine Risikoversicherungen, die Säule 3a und Kollektivverträge der beruflichen Vorsorge', 's')
add(k, 'Wann ist trotz kleiner Prämie zu identifizieren?',
    'Sobald Verdachtsmomente auf Geldwäscherei oder Terrorismusfinanzierung bestehen – dann gilt kein Schwellenwert', 'p')
add(k, 'Was ist die wirtschaftlich berechtigte Person?',
    'Die natürliche Person, der das Geld tatsächlich gehört – sie ist festzustellen, auch wenn eine andere unterschreibt', 'm')
add(k, 'Was ist bei grossen Abschlüssen zusätzlich abzuklären?',
    'Die Herkunft der Mittel – belegt etwa mit der Steuerveranlagung, einem Lohnausweis oder einem Vermögensnachweis', 'm')
add(k, 'Warum genügt die blosse Frage nach der Herkunft nicht?',
    'Weil die Antwort plausibel sein und belegt werden muss – und das Ergebnis gehört in die Akten', 's')
add(k, 'Wie lange sind die Unterlagen aufzubewahren?',
    f"{GW['aufbewahrung_jahre']} Jahre nach dem Ende der Geschäftsbeziehung", 'm')
add(k, 'An wen geht die Meldung bei Verdacht?',
    'An die Meldestelle für Geldwäscherei des Bundes – nicht an die FINMA und nicht an die Polizei', 'e')
add(k, 'Wofür steht die Abkürzung MROS?',
    'Money Laundering Reporting Office Switzerland – die Meldestelle für Geldwäscherei', 'm')
add(k, 'Welcher Verdacht löst die Meldepflicht aus?',
    'Der begründete Verdacht auf Geldwäscherei, auf Vermögen aus einem Verbrechen oder auf Terrorismusfinanzierung – '
    'Beweise braucht es nicht', 's')
add(k, 'Was geschieht mit den Vermögenswerten nach einer Meldung?',
    'Sie werden gesperrt, bis die Meldestelle entschieden hat', 's')
add(k, 'Darf die Kundschaft über die Meldung informiert werden?',
    'Nein – es gilt ein Informationsverbot, und ein Verstoss ist strafbar', 'p')
add(k, 'Was schützt die meldende Person?',
    'Wer in guten Treuen meldet, haftet weder straf- noch zivilrechtlich', 'p')

# ============================================ AIA und FATCA
k = kap('vbv-aia', 'AIA und FATCA', 'Steuerdaten über die Grenze')
AI = K['aia']
add(k, 'Wofür steht die Abkürzung AIA?', 'Automatischer Informationsaustausch', 'e')
add(k, 'Wie heisst der AIA mit vollem Namen?',
    'Automatischer Informationsaustausch über Finanzkonten', 'e')
add(k, 'Wie heisst der Standard hinter dem AIA?',
    'Der Common Reporting Standard der OECD – kurz CRS', 'm')
add(k, 'Wann kommt der AIA zum Einsatz?',
    'Wenn die Kundschaft in einem Partnerstaat steuerlich ansässig ist – dann geht die Meldung über die '
    'Eidgenössische Steuerverwaltung in diesen Staat', 'm')
add(k, 'Wann kommt FATCA zum Einsatz?',
    'Wenn eine US-Person beteiligt ist – dann geht die Meldung an die Steuerbehörde der USA', 'm')
add(k, 'Welches Abkommen gilt wann?',
    'Bei US-Bezug FATCA, bei jedem anderen Auslandbezug der AIA – ohne Auslandbezug gibt es keine Meldung', 's')
add(k, 'Was ist der automatische Informationsaustausch?',
    'Der internationale Standard, nach dem Finanzinstitute Daten über Konten von im Ausland steuerpflichtigen '
    'Personen weitergeben', 'e')
add(k, 'Seit wann gilt der AIA in der Schweiz?',
    f"Die Rechtsgrundlagen sind am 1. Januar {AI['inkraft_schweiz']} in Kraft getreten", 'm')
add(k, 'Wie viele Staaten machen beim AIA mit?', 'Über 100 Staaten, darunter alle wichtigen Finanzplätze', 'e')
add(k, 'Welcher grosse Staat macht beim AIA nicht mit?',
    'Die USA – sie gehen mit FATCA einen eigenen Weg', 'm')
add(k, 'Über welchen Weg laufen die Daten?', AI['weg'], 'm')
add(k, 'Wie oft wird gemeldet?', 'Einmal im Jahr, jeweils im Herbst des Folgejahres', 'm')
add(k, 'Was bedeutet der AIA für das Bankgeheimnis?',
    'Gegenüber ausländischen Steuerbehörden gibt es keines mehr – im Inland gilt es weiter', 's')
add(k, 'Welche Versicherungsverträge sind meldepflichtig?', AI['meldepflichtig'], 'm')
add(k, 'Welche Verträge sind nicht meldepflichtig?',
    'Reine Risikoversicherungen ohne Rückkaufswert und die gebundene Vorsorge', 's')
add(k, 'Was ist eine Selbstauskunft?',
    'Die Erklärung der Kundschaft über ihre steuerliche Ansässigkeit samt Steuernummer – ohne sie kein Vertrag', 'm')
add(k, 'Wofür steht die Abkürzung FATCA?', AI['fatca_lang'], 'e')
add(k, 'Was ist FATCA?',
    'Ein Gesetz der USA, das ausländische Finanzinstitute verpflichtet, Konten von US-Personen den '
    'amerikanischen Steuerbehörden zu melden', 'e')
add(k, 'Seit wann gilt FATCA für die Schweiz?',
    f"Das Abkommen mit den USA ist {AI['fatca_inkraft']} in Kraft getreten", 'p')
add(k, 'Wer gilt als US-Person?',
    'Wer die US-Staatsbürgerschaft hat, in den USA geboren ist, eine Green Card besitzt oder sich regelmässig '
    'dort aufhält', 'm')
add(k, 'Wie meldet die Schweiz heute nach FATCA?',
    'Nach Modell 2: mit Zustimmung der Kundschaft direkt an die US-Steuerbehörde, sonst über ein Amtshilfeersuchen', 'p')
add(k, 'Was unterscheidet den AIA von FATCA?',
    'Der AIA ist gegenseitig und gilt zwischen über hundert Staaten; FATCA ist einseitiges US-Recht und betrifft '
    'nur US-Personen', 's')
add(k, 'Warum geht das die Lebensversicherung an?',
    'Weil bei jedem Abschluss die steuerliche Ansässigkeit zu klären ist – auch eine Verbindung in die USA', 's')
